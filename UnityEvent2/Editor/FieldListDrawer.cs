    ...
    public class FieldListDrawer : PropertyDrawer
    {
        ...
        public void GetAllEvent(Component comp, ref List<string> fieldList)
        {
            ...
            for (int i = 0; i < compProperty.Length; i++)
            {
                ...
                PropertyInfo prop = compProperty[i];

                if (!prop.CanRead)
                    continue;

                if (IsDangerousUnityProperty(comp, prop))
                    continue;
        ...
        static bool IsDangerousUnityProperty(Component comp, PropertyInfo prop)
        {
            // Indexers
            if (prop.GetIndexParameters().Length > 0)
                return true;

            // NavMeshAgent (major offender)
            if (comp is UnityEngine.AI.NavMeshAgent)
            {
                return prop.Name is
                    "isStopped" or
                    "remainingDistance" or
                    "path" or
                    "hasPath" or
                    "pathPending" or
                    "velocity" or
            ...
            if (comp is Renderer)
            {
                return prop.Name is "material" or "materials" or "bounds";
            }
